...
$(document).ready(function () {
    $('#articutLawArticle').change(function () {
        for (var i = 0; i < $('.articutLawArticle').length; i++) {
            if (this.checked) {
                $($('.articutLawArticle')[i]).html(DOMPurify.sanitize('<span class="badge badge-success">' + $($('.articutLawArticle')[i]).text() + '</span>', { SAFE_FOR_JQUERY: true }));
            } else {
    ...
    $('#articutCrime').change(function () {
        for (var i = 0; i < crime_list.length; i++) {
            ...
            for (var j = 0; j < $('body .card-body').length; j++) {
                if (this.checked) {
                    $('body .card-body').eq(j).html(DOMPurify.sanitize($('body .card-body').eq(j).html().replace(new RegExp(escapeRegExp(text), "g"), '<span class="badge badge-danger">' + text + '</span>'), { SAFE_FOR_JQUERY: true }));
                } else {
                    $('body .card-body').eq(j).html(DOMPurify.sanitize($('body .card-body').eq(j).html().replace(new RegExp(escapeRegExp('<span class="badge badge-danger">' + text + '</span>'), "g"), text), { SAFE_FOR_JQUERY: true }));
                }
            }
        }
        if (this.checked) {
    ...
    $('#articutCriminalResponsibility').change(function () {
        for (var i = 0; i < criminal_responsibility_list.length; i++) {
            ...
            for (var j = 0; j < $('body .card-body').length; j++) {
                if (this.checked) {
                    $('body .card-body').eq(j).html(DOMPurify.sanitize($('body .card-body').eq(j).html().replace(new RegExp(escapeRegExp(text), "g"), '<span class="badge badge-warning">' + text + '</span>'), { SAFE_FOR_JQUERY: true }));
                } else {
                    $('body .card-body').eq(j).html(DOMPurify.sanitize($('body .card-body').eq(j).html().replace(new RegExp(escapeRegExp('<span class="badge badge-warning">' + text + '</span>'), "g"), text), { SAFE_FOR_JQUERY: true }));
                }
            }
        }
        if (this.checked) {
    ...
    $('#articutEventRef').change(function () {
        for (var i = 0; i < event_ref_list.length; i++) {
            ...
            for (var j = 0; j < $('body .card-body').length; j++) {
                if (this.checked) {
                    $('body .card-body').eq(j).html(DOMPurify.sanitize($('body .card-body').eq(j).html().replace(new RegExp(escapeRegExp(text), "g"), '<span class="badge badge-primary">' + text + '</span>'), { SAFE_FOR_JQUERY: true }));
                } else {
                    $('body .card-body').eq(j).html(DOMPurify.sanitize($('body .card-body').eq(j).html().replace(new RegExp(escapeRegExp('<span class="badge badge-primary">' + text + '</span>'), "g"), text), { SAFE_FOR_JQUERY: true }));
                }
            }
        }
        if (this.checked) {
...
function init(result, lawsResult, judHistory, judInfo) {
    ...
    content += '</div></div>';
    $('#articutLawsResult').html(DOMPurify.sanitize(content, { SAFE_FOR_JQUERY: true }));
